...
`BEDROCK_AUTH_MODE=bearer` is a declared/validated value for future Mantle support; for now, workers in `bearer` mode fall through to the standard credential check (key / auth.json).
...
| `AWS_REGION` | any Bedrock-enabled region | `us-east-1` (SDK default) |

`AWS_REGION` should be set explicitly; the probe uses `us-east-1` as a fallback but may fail for models not available in that region.
